...
void initClocks(){
  ...
  for(int i = 0; i < clockX.length; i++){
    ...
    while(isExist(cabbageX[i],cabbageY[i],clockX[i],clockY[i]));
  }
}
boolean isExist(float cabbageX,float cabbageY,float clockX,float clockY){
  for(int i=0; i<6; i++){
    if( cabbageX==clockX && cabbageY==clockY ){
     return true;
    }       
   }  
  return false;
...
boolean isHit(float x1,float x2,float y1,float y2){
      ...
      if(playerHealth < PLAYER_MAX_HEALTH
        ...
        && y1 < y2 + SOIL_SIZE) { // r1 bottom edge past r2 top
       return true;
    }
  return false;
}
...
void draw() {
  ...
  switch (gameState) {
    ...
    for(int i = 0; i < clockX.length; i++){
      ...
      // Requirement #3: Use boolean isHit(...) to detect collision   ////////////
     if( isHit(clockX[i], playerX, clockY[i], playerY)) {
        addTime(900);
    ...
    for(int i = 0; i < soldierX.length; i++){
      ...
      // Requirement #3: Use boolean isHit(...) to detect collision
      if(isHit(soldierX[i],playerX,soldierY[i],playerY)){ 
...
color getTimeTextColor(int frames){        // Requirement #5
 //frames = gameTimer; 
  if (frames>=120*60){
    return #00ffff;
  }else if(frames<120*60 && frames>=60*60){
    return #ffffff;
  }else if(frames<60*60&&frames>=30*60){
    return #ffcc00;
  }else if(frames<30*60&&frames>=10*60){
    return #ff6600;
  }else {
    return #ff0000;
  }
